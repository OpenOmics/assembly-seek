# <code>assembly-seek <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>assembly-seek <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking assembly-seek pipeline output directory is fast and easy! In its most basic form, <code>assembly-seek <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./assembly-seek unlock [-h] --output OUTPUT
```
...
assembly-seek unlock --output /data/$USER/output
```